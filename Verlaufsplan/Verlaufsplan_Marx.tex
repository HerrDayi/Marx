...
% === LERNZIELE (OHNE KASTEN DARÜBER - NACH REGELN VON DR. UHTES) ===
\noindent
{\footnotesize\textbf{\color{primaryBlue}Hauptlernziel (HLZ):} Die SuS erschließen, dass nach Karl Marx die moderne industrielle Produktionsweise das menschliche Gattungswesen verkehrt, indem sie die schöpferische Arbeit von einem Akt freier Selbstverwirklichung in fremdbestimmte Zwangsarbeit transformiert und den Menschen vom Produkt, von der Tätigkeit und von seinen Mitmenschen entfremdet.}

\vspace{0.05cm}
\noindent
{\footnotesize\textbf{\color{primaryBlue}Teillernziele (TLZ):} Die SuS \dots \enskip
\textbf{TLZ 1 (AFB I):} \textit{erfassen} am Beispiel von Charlie Chaplins Fließbandarbeit das Phänomen der Technisierung und formulieren das Problem der Herrschaft der Maschine über den Menschen. \enskip$\cdot$\enskip
\textbf{TLZ 2 (AFB II):} \textit{rekonstruieren} Marx’ Argumentationskette, indem sie vier isolierte Kernaussagen in ihre kausale Abfolge bringen (vom Gattungswesen über Produkt- und Tätigkeitsentfremdung bis zum Paradoxon). \enskip$\cdot$\enskip
\textbf{TLZ 3 (AFB II):} \textit{analysieren} anhand des Originaltextes die vier Dimensionen der entfremdeten Arbeit und weisen den Textstellen ihre argumentative Funktion zu. \enskip$\cdot$\enskip
\textbf{TLZ 4 (AFB II):} \textit{ordnen} Marx’ Diagnose in das Spannungsfeld des Advanced Organizers ein (Abgrenzung zu Gehlens Entlastungsthese; Kultur als Trugbild). \enskip$\cdot$\enskip
\textbf{TLZ 5 (AFB III / Res.):} \textit{beurteilen} die Aktualität von Marx’ Paradoxon für die heutige Arbeits- und Konsumwelt (Meme-Gestaltung).}
